...
<div class="content">
    <div class="order_address">
        ...
        <div class="br"></div>
        <label for="textarea_details_address" id="label_details_address">Full address</label><span class="mustValue">*</span>
        <textarea id="textarea_details_address">${requestScope.detailsAddress}</textarea>
        <div class="br"></div>
        <label for="input_order_receiver" id="label_order_receiver">Consignee's name</label><span class="mustValue">*</span>
        <input id="input_order_receiver" type="text" value="${requestScope.order_receiver}" maxlength="20"/>
        <div class="br"></div>
        <label for="input_order_phone" id="label_order_phone">Mobile phone number</label><span class="mustValue">*</span>
    ...
    <div class="order_info">
        ...
        <table class="table_order_orderItem">
            ...
            <c:forEach items="${requestScope.orderItemList}" var="orderItem" varStatus="i">
                <tr class="tr_shop">
                    <td><span class="span_shopTitle">shop：</span><span
                ...
                <tr class="tr_userMessage">
                    <td><label for="input_userMessage_${i.count}">Leave a message to the seller：</label><textarea maxlength="500"
                                                                                          id="input_userMessage_${i.count}"
                                                                                          placeholder="Optional: The content has been negotiated and confirmed with the seller"
                ...
                <tr class="tr_orderCount">
                    <td colspan="3"></td>
                    <td><span class="span_price_name">Total store (including shipping fee)</span><span
        ...
        <div class="order_count_div_main">
            <div class="order_count_div_content">
                <h1 class="order_count_div_price">
                    <span class="order-title">actual payment：</span><span class="realPay-price_unit">￥</span><span
                        class="realPay-price">${requestScope.orderTotalPrice}0</span>
                </h1>
                <h1 class="order_count_div_address">
                    <span class="order-title">sent to：</span><span class="order-value address_province"></span><span
                ...
                <h1 class="order_count_div_phone">
                    <span class="order-title">consignee：</span><span
    ...
    <script>
        function payOne() {
            ...
            $.ajax({
                ...
                error: function () {
                    alert("There was a problem with the order submission, please resubmit!");
                    location.reload(true);
                }
            });
        }
    ...
    <div class="order_info_last">
        <c:choose>
            <c:when test="${requestScope.orderItemList[0].productOrderItem_id != null}">
                <a href="javascript:void(0)" title="submit your order" class="go-btn" onclick="payList()">submit your order</a>
            </c:when>
            <c:otherwise>
                <a href="javascript:void(0)" title="submit your order" class="go-btn" onclick="payOne()">submit your order</a>
            </c:otherwise>
        </c:choose>
    </div>
</div>
