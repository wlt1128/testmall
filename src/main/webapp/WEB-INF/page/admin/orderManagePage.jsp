...
<head>
    <script>
                        ...
                        for (var i = 0; i < data.productOrderList.length; i++) {
                            ...
                            switch (data.productOrderList[i].productOrder_status) {
                                case 0:
                                    ...
                                    productOrderStatusTitle = "wait for the buyer to pay";
                                    productOrderStatus = "wait for the buyer to pay";
                                    break;
                                case 1:
                                    productOrderStatusClass = "td_warn";
                                    productOrderStatusTitle = "The buyer has paid and is waiting for the seller to ship the goods";
                                    productOrderStatus = "wait for the seller to ship the goods";
                                    break;
                                case 2:
                                    productOrderStatusClass = "td_wait";
                                    productOrderStatusTitle = "The seller has shipped the goods and is waiting for confirmation from the buyer";
                                    productOrderStatus = "wait for the buyer s confirmation";
                                    break;
                                case 3:
                                    productOrderStatusClass = "td_success";
                                    productOrderStatusTitle = "the transaction was successful";
                                    productOrderStatus = "the transaction was successful";
                                    break;
                                default:
                                    productOrderStatusClass = "td_error";
                                    productOrderStatusTitle = "transaction closed";
                                    productOrderStatus = "transaction closed";
                                    break;
                            }
        ...
        function getChildPage(obj) {
            //设置样式
            $("#div_home_title").children("span").text("order details");
...
<div class="frm_div text_info">
    <div class="frm_group">
        <label class="frm_label" id="lbl_productOrder_code" for="input_productOrder_code">order number</label>
        <input class="frm_input" id="input_productOrder_code" type="text" maxlength="20"/>
        <input class="frm_btn" id="btn_productOrder_submit" type="button" value="query"/>
            ...
            <td>
                <c:choose>
                    <c:when test="${productOrder.productOrder_status==0}">
                        <span class="td_await" title="等待买家付款">wait for the buyer to pay</span>
                    </c:when>
                    <c:when test="${productOrder.productOrder_status==1}">
                        <span class="td_warn" title="买家已付款，等待卖家发货">wait for the seller to ship the goods</span>
                    </c:when>
                    <c:when test="${productOrder.productOrder_status==2}">
                        <span class="td_wait" title="卖家已发货，等待买家确认">wait for the buyer s confirmation</span>
                    </c:when>
                    <c:when test="${productOrder.productOrder_status==3}">
                        <span class="td_success" title="交易成功">the transaction was successful</span>
                    </c:when>
                    <c:otherwise><span class="td_error" title="交易关闭">transaction closed</span></c:otherwise>
                </c:choose>
            </td>
